...
static nvm_err_t nvm_esp_open(void) {
    ...
    if (nvm_esp_partition == NULL) {
        ...
        ESP_LOGI(TAG, "found nvm partition '%s' at offset 0x%" PRIx32 " with size 0x%" PRIx32" with sector size 0x%x" , nvm_esp_partition->label,
                 nvm_esp_partition->address, nvm_esp_partition->size, NVM_SECTOR_SIZE);
    }
...
static nvm_err_t nvm_esp_read(uint32_t sector_index, uint8_t *sector_buffer) {
    esp_err_t err = esp_partition_read_raw(nvm_esp_partition, sector_index * NVM_SECTOR_SIZE, sector_buffer, NVM_SECTOR_SIZE);
    if (err != ESP_OK) {
...
static nvm_err_t nvm_esp_write(uint32_t sector_index, uint8_t *sector_buffer) {
    esp_err_t err = esp_partition_write_raw(nvm_esp_partition, sector_index * NVM_SECTOR_SIZE, sector_buffer, NVM_SECTOR_SIZE);
    if (err != ESP_OK) {
...
static nvm_err_t nvm_esp_erase(uint32_t sector_index, uint32_t sector_count) {
    esp_err_t err = esp_partition_erase_range(nvm_esp_partition, sector_index * NVM_SECTOR_SIZE, sector_count * NVM_SECTOR_SIZE);
    if (err != ESP_OK) {
